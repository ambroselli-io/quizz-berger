...
export default function Home() {
  ...
  // Randomize only on the client (after hydration) to avoid SSR/client mismatch.
  const [themes, setThemes] = useState(() => quizz);
  const [random, setRandom] = useState(0);
  useEffect(() => {
    setThemes([...quizz].sort(() => 0.5 - Math.random()));
    setRandom(Math.round(Math.random() * 15));
  }, []);
