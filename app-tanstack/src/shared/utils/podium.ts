import type {
  MinimalUserAnswerWithScorePerThemeAndMax,
  PodiumData,
  PodiumDataWithPercentAndHeightAndHighest,
} from '@api/src/types/answer';
...
export const getPicName = (podiumised: Array<PodiumDataWithPercentAndHeightAndHighest>) =>
  podiumised
    ...
    .map(
      (data) =>
        `${data.percent}${data.pictures
          .filter((p) => p !== null)
          .map((pic: string) => pic.slice(0, 2))
          .join('')}`,
    )
    .join('');
      ...
      candidates = candidates.map((c) =>
        c.id !== sameTotal.id
          ...
          : {
              ...
              pictures: [...c.pictures, candidate.picture]
                .filter((p): p is string => p !== null)
                .sort((pic1, pic2) => pic1.localeCompare(pic2)),
              colors: [...c.colors, candidate.color]
                .filter((c): c is string => c !== null)
                .sort((color1, color2) => color1.localeCompare(color2)),
            },
      );
    } else {
      highest = newHighest;
