...
Source: https://github.com/petergyang/no-ai-slop (SKILL.md + eval.md, vendored). The word and pattern lists below are English. This repo publishes in French, so also apply `french.md`, which maps the same patterns onto French.
...
For French drafts, read `french.md` before step 2 and check against its list as well as `eval.md`.
