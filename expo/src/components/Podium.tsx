...
const Podium = ({ podiumised, title }: PodiumProps) => {
  ...
  return (
    <View className="flex-1">
      {title && (
        <Text className="mb-2 px-4 text-lg font-bold text-quizz-dark" style={{ fontFamily: "Merriweather_700Bold" }}>
          {title}
        </Text>
      )}
      <ScrollView
      ...
      >
        {podiumised.map(({ pseudos, pictures, height, percent, colors }, index) => {
          ...
          return (
            <View key={`${height}-${pseudos.join(",")}`} className="items-center mx-0.5" style={{ width: barWidth }}>
              {/* Avatar(s) */}
              ...
              >
                <Text className="text-center font-bold" style={{ fontSize: Math.max((percent / 100) * 18, 10) }}>
                  {percent}%
                </Text>
              </View>
